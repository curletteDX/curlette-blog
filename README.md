...
Choose your region and copy the appropriate environment file:

For US region:
```bash
cp .env.example.us .env
```

For EU region:
```bash
cp .env.example.eu .env
```
...
> **Note about `UNIFORM_PREVIEW_SECRET`**: This is not a Uniform credential but a custom secret you can set to any value. However, if you change it from `light-starter`, you must also update the preview URL in your Uniform project settings to match. See [Configure Preview URLs](https://docs.uniform.app/docs/guides/composition/visual-editing/preview-mode#configure-preview-urls) for instructions on updating the preview URL to: `https://<YOUR_HOSTNAME>/api/preview?secret=<YOUR_PREVIEW_SECRET>`
